...
#================ Intialize Values ====================#
# "C:/Users/mfm160330/OneDrive - The University of Texas at Dallas/ADAS data/FaceAndEyes/FE2019-5-22/"
FaceIDFileAndWriteLoc = "C:/Users/mfm160330/OneDrive - The University of Texas at Dallas/ADAS data/FaceAndEyes/FE2019-7-15/"
...
pickle_in_path = "SOMEPATHHERE"

if __name__ == "__main__":
    label(pickle_in_path, FaceIDFileAndWriteLoc, OutputFilesReadLoc)
    exit()
...
def label(calib_pickle_path, id_file_path, visualize_file_path, kabsch_transform_path, angles_id_file_output_path):
    IDFileLoc = id_file_path
    VisualizeFile = visualize_file_path
    AnglesIdFileName = angles_id_file_output_path
    KabaschRotTrans = kabsch_transform_path

    FrameIndexNumber = 3  # In image name, after which dash is the frame number located
    print("Using frame index %d" % (FrameIndexNumber))

    Categories = ["a- 4", "b- 1", "c- 8", "d- 2", "e- 13", "f- 5", "g- 9", "h- 11", "i- 6", "j- 20",
                  "k- 19", "l- 18", "m- 21", "n- 17", "o- 16", "p- 14", "q- 3", "r- 7", "s- 10", "t- 12", "u- 15"]

    pickle_in = open(calib_pickle_path, "rb")
    firstMarkerIndx = 11  # Indx of the first marker of the target markers

    print("Choosing markers after firstMarkerIndx %d" % firstMarkerIndx)
    ...
    with open(IDFileLoc, "r") as csvfile:
        ...
        for row in readCSV:
            ...
            IDs.append(row[1])
            labels.append(row[2])
            Face_X1.append(row[3]), Face_Y1.append(
                row[4]), Face_X2.append(row[5]), Face_Y2.append(row[6])
            LEye_X1.append(row[7]), LEye_Y1.append(
                row[8]), LEye_X2.append(row[9]), LEye_Y2.append(row[10])
            REye_X1.append(row[11]), REye_Y1.append(
                row[12]), REye_X2.append(row[13]), REye_Y2.append(row[14])
    ...
    pickle_in2 = open(KabaschRotTrans, "rb")
    ...
    # Angle labels contain X,Y,X,Rho,Theta,Phi
    AngleLabels = ConvertToSpherical_np(XYZlc)
    ...
    csvfile = open(AnglesIdFileName, 'w+')
    csvfile.write(csv_header+"\n")

    for i in range(XYZlc.shape[0]):
        with open(AnglesIdFileName, 'a+') as csvfile:
            ...
            filewriter.writerow([DataSets[i], IDs[i], AngleLabels[i][0], AngleLabels[i][1], AngleLabels[i][2], XYZcomCalib[i][0], XYZcomCalib[i][1], XYZcomCalib[i][2], Xlabel[i][0], Ylabel[i]
                                 [0], Zlabel[i][0], Face_X1[i], Face_Y1[i], Face_X2[i], Face_Y2[i], LEye_X1[i], LEye_Y1[i], LEye_X2[i], LEye_Y2[i], REye_X1[i], REye_Y1[i], REye_X2[i], REye_Y2[i], labels[i]])
