...
def get_cmd_line_args():
    ...
    parser.add_argument(
        "visualize_output", help="Path of output from visualize_frames.py", type=str)
    parser.add_argument("road_to_back_normalized",
                        help="Path of output from normalize_road.py", type=str)
...
def calc_spherical(pos):
    ...
    sph[1] = np.arctan2(np.sqrt(xy), pos[2])
...
def get_road_data(POSE_ALL, R2B, offset, face_csv_file):
    ...
    # transposing so we can iterate over (what were) rows rather of columns
    R2B = R2B.T
    POSE_ALL = POSE_ALL.T
    ...
    while vis_row_ptr < vis_frames and road_row_ptr < road_frames:
        ...
        if str(road_row['detectionId']) == str(np.nan) or str(vis_row['x']) == str(np.nan):
            ...
            road_trans = np.asarray(
                (road_row['projX'], road_row['projY'], road_row['projZ']))
            # transforming OpenGL coordinates from visualize_2 script to ApriLTag coordinate system
            vis_trans = np.asarray(
                (-vis_row['z'], -vis_row['x'], vis_row['y']))
...
def main():
    ...
    # creating pandas dataframe from Road to Back CSV file
    R2B = pd.read_csv(ROAD_TO_BACK_FILE, sep='\t')
    # gettng head pose matrix from matlab file
    POSE_ALL = pd.read_csv(VISUALIZE, sep='\t')

    if mycmd:
        # opening output file for face csv data
        face_csv_file = open("output/ContGazeIntialLabelsAllFrames.csv", 'w+')
        print("Syncing %s and %s with purported offset %d" %
              (args.road_to_back_normalized, args.visualize_output, offset))
    else:
        # opening output file for face csv data
        face_csv_file = open(
            "../../output/ContGazeIntialLabelsAllFrames.csv", 'w+')
        print("Syncing %s and %s with purported offset %d" %
              (STD_VIS, STD_ROAD, offset))
