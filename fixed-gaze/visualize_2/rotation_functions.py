...
def eul2mat(angles):
    ...
    rot_y = np.matrix([[math.cos(angles[0]), -math.sin(angles[0]), 0],
                       [math.sin(angles[0]), math.cos(angles[0]), 0], [0, 0, 1]])
    rot_p = np.matrix([[math.cos(angles[1]), 0, -math.sin(angles[1])],
                       [0, 1, 0], [math.sin(angles[1]), 0, math.cos(angles[1])]])
    rot_r = np.matrix([[1, 0, 0], [0, math.cos(
        angles[2]), -math.sin(angles[2])], [0, math.sin(angles[2]), math.cos(angles[2])]])
...
def mat2eul(input):
    ...
    angles[2] = math.atan2((input[0, 2]*s-input[1, 2]*c),
                           (-input[0, 1]*s+input[1, 1]*c))
    return angles
...
def orient_square(loc, quat):
    sqr = np.array([[0.02, 0.02, 0], [-0.02, 0.02, 0],
